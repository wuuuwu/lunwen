...
- 按当前课程的任务、学习目标和课程知识评价普通课程论文，不引入课程之外的背景要求。
- 课程流程不采集或保存学生专业，封面上的相关字段在进入模型前会被本地遮蔽。
- 扫描所选文件夹顶层的 `1–100` 个 PDF，不递归读取子目录，并按文件名顺序逐篇评测。
- 支持停止、从检查点继续，以及只重试失败论文；已完成论文不会重复调用模型。
- 自动提取姓名、学号和正文可见题目；PDF 隐藏标题只用于核对，不能单独成为题目。
- 低置信度或缺失字段会醒目标记，可逐篇核对，也可对当前格式批次执行完全本地的“一键重新检查”。
- 评分 Reviewer 和 Meta Reviewer 不接收姓名、学号或原始文件路径；专用元数据提取步骤
...
```text
姓名_学号_题目_课程论文评测报告.pdf
```

文件名会自动清理 Windows 非法字符并限制长度。姓名、学号或题目仍待核对时，首次报告使用：

```text
待核对论文__待核对__任务ID前8位_课程论文评测报告.pdf
```

人工确认后，报告会在本地重建并改为标准的姓名、学号、题目格式；若出现重名，程序会追加
稳定短标识，绝不覆盖已有报告。输出目录同时包含：
...
汇总表包含原文件名、姓名、学号、题目、待核对字段、人工核对状态、各维度分数、总分、
...
打开，全程不调用模型；当前格式批次也可补生成。如果文件正在 Excel 中占用，评测不会中断，关闭成绩表后
再次点击即可更新。
...
- 通用 Rubric 版本为 `0.1.1-experimental`，尚未完成具体课程的教育测量效度验证。
