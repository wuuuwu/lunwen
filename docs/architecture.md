...
```text
INGEST -> EVIDENCE -> SCORING -> AUDIT
       -> 3 INITIAL EXPERTS -> OPTIONAL 2 SUPPLEMENTAL EXPERTS
       -> SYNTHESIS -> REPORT -> OPTIONAL HUMAN REVIEW / LOCAL REPORT REFRESH
```
...
overridden by the diagnostic score or expert votes. A suspected hard rule or `unable_to_assess`
creates a post-report human-review item without interrupting the remaining AI stages. The
meta-reviewer writes only a summary; Python recomputes and audits the score, human decisions,
votes, and final decision path. Human decisions refresh the report locally without another model
call.
...
Prompts instruct the model to treat external documents as data rather than instructions. Tools are
registered by trusted code and filtered through a static allowlist before every execution; this is
a deterministic tool boundary, not a guarantee that a model will semantically ignore every prompt
injection attempt.
...
post-report human-review requirements for policy and integrity decisions remain unchanged.
