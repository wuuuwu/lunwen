...
- 课程 Rubric 和 Reviewer Profile 升级为 `0.1.1-experimental`，课程内容评价只依据课程任务、学习目标和核心知识点。
- 课程提交元数据升级为 Schema 2.0，仅保留姓名、学号和题目；批次清单升级为 Schema 2。
- 新报告、汇总表、界面和文件名不再包含学生专业，封面中的相关字段在模型调用前本地遮蔽。
- Schema 1 课程批次不自动迁移、不重写，需使用旧版本查看；本科毕业论文流程保持不变。
